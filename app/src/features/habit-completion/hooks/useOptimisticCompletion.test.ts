...
describe('useOptimisticCompletion', () => {
  ...
  it('should create optimistic update immediately on toggle', async () => {
    ...
    expect(result.current.getOptimisticState(1, new Date().toISOString().split('T')[0], false)).toBe(true);
    expect(result.current.hasOptimisticUpdate(1, new Date().toISOString().split('T')[0])).toBe(true);
  ...
  it('should call API and clean up optimistic update on success', async () => {
    ...
    expect(result.current.hasOptimisticUpdate(1, new Date().toISOString().split('T')[0])).toBe(false);
  });
  ...
  it('should rollback optimistic update on API error', async () => {
    ...
    expect(result.current.getOptimisticState(1, new Date().toISOString().split('T')[0], false)).toBe(false);
    expect(result.current.hasOptimisticUpdate(1, new Date().toISOString().split('T')[0])).toBe(false);
  ...
  it('should handle API rollback callback correctly', async () => {
    ...
    mockCompletionApi.toggleCompletionOptimistic.mockImplementation((habitId, request, signal, onRollback) => {
