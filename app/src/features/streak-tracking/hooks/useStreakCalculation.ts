...
export function useStreakCalculation(habitId?: number) {
  ...
  const recalculateStreak = useMutation({
    ...
    onSuccess: (updatedStreak, targetHabitId) => {
      ...
      queryClient.setQueryData(
        streakQueryKeys.byHabit(targetHabitId),
        updatedStreak
      );
  ...
  const updateStreakInCache = useCallback((updatedStreak: StreakResponse) => {
    queryClient.setQueryData(
      streakQueryKeys.byHabit(updatedStreak.habitId),
      updatedStreak
    );
  }, [queryClient]);

  // Invalidate streak data (force refresh)
  const invalidateStreak = useCallback((targetHabitId: number) => {
    queryClient.invalidateQueries({
  ...
  const getCachedStreak = useCallback((targetHabitId: number) => {
    return queryClient.getQueryData<StreakResponse>(
      streakQueryKeys.byHabit(targetHabitId)
    );
  }, [queryClient]);

  // Prefetch streak data
  const prefetchStreak = useCallback((targetHabitId: number) => {
...
export function useTrackerStreaks(trackerId?: number) {
  ...
  const recalculateAllStreaks = useMutation({
    ...
    onSuccess: (updatedStreaks, targetTrackerId) => {
      queryClient.setQueryData(
        streakQueryKeys.byTracker(targetTrackerId),
        updatedStreaks
      );
      ...
      updatedStreaks.forEach(streak => {
        queryClient.setQueryData(
          streakQueryKeys.byHabit(streak.habitId),
          streak
        );
      });
