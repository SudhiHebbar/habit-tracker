...
export const IconSelector: React.FC<IconSelectorProps> = ({
  ...
  const handleIconKeyDown = useCallback(
    (event: React.KeyboardEvent, iconId: string) => {
  ...
  const renderCategory = useCallback(
    (category: IconCategoryInfo & { icons: IconOption[] }) => {
  ...
  return (
    <div className={`${styles.iconSelector} ${styles[size]} ${className}`}>
        ...
        <div className={styles.selectedIconDisplay}>
          ...
          <div className={styles.selectedIconInfo}>
            {(() => {
              ...
              return icon ? (
                <>
                  <div className={styles.selectedIconPreview}>{renderIconButton(icon)}</div>
      ...
      {/* Icon Categories */}
      {showCategories ? (
        <div className={styles.categories}>{categoriesWithIcons.map(renderCategory)}</div>
      ) : (
      ...
      <div className={styles.quickStats}>
        <span className={styles.stat}>Categories: {IconLibrary.getCategories().length}</span>
        <span className={styles.stat}>Popular: {IconLibrary.getPopularIcons().length}</span>
        {IconLibrary.getNewIcons().length > 0 && (
          <span className={styles.stat}>New: {IconLibrary.getNewIcons().length}</span>
        )}
      </div>
