...
export const ColorPalette: React.FC<ColorPaletteProps> = ({
  ...
  const renderColorSwatch = useCallback((color: ColorOption) => {
  ...
  const renderCategory = useCallback((category: ColorCategory & { colors: ColorOption[] }) => {
    ...
    <div className={`${styles.colorPalette} ${styles[size]} ${className}`}>
      ...
      {showCategories ? (
        <div className={styles.categories}>
          {categoriesWithColors.map(renderCategory)}
        </div>
      ) : (
      ...
      <div className={styles.resultsInfo}>
        {filteredColors.length === 0 ? (
          ...
          <p className={styles.resultsCount}>
            {filteredColors.length} of {ColorSystem.getCategories().reduce((total, cat) => total + cat.colors.length, 0)} colors
