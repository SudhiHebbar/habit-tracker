  ...
  return (
    <div className={`${styles.frequencySelector} ${className}`}>
      ...
      <div className={styles.frequencyOptions}>
        {PRESET_FREQUENCIES.map(renderFrequencyOption)}
      </div>
            ...
            {!localCustomFrequency.timesPerMonth && (
              <div className={styles.customControl}>
                <input
                  ...
                  onChange={(e) => handleCustomFrequencyChange({ timesPerWeek: parseInt(e.target.value) })}
                  disabled={disabled}
                  className={styles.rangeInput}
                />
                <span className={styles.rangeValue}>
                  {localCustomFrequency.timesPerWeek} time{localCustomFrequency.timesPerWeek !== 1 ? 's' : ''} per week
                </span>
              </div>
            )}
          </div>
            ...
            {localCustomFrequency.timesPerMonth && (
              <div className={styles.customControl}>
                <input
                  ...
                  onChange={(e) => handleCustomFrequencyChange({ timesPerMonth: parseInt(e.target.value) })}
                  disabled={disabled}
                  className={styles.rangeInput}
                />
                <span className={styles.rangeValue}>
                  {localCustomFrequency.timesPerMonth} time{localCustomFrequency.timesPerMonth !== 1 ? 's' : ''} per month
                </span>
              </div>
            )}
          </div>
          ...
          {!localCustomFrequency.timesPerMonth && (
            <div className={styles.specificDays}>
              ...
              <p className={styles.daysDescription}>
                Leave empty for any {localCustomFrequency.timesPerWeek} day{localCustomFrequency.timesPerWeek !== 1 ? 's' : ''} of the week
              </p>
              ...
              {localCustomFrequency.specificDays && localCustomFrequency.specificDays.length > 0 && (
                <p className={styles.selectedDaysText}>
                  Selected: {localCustomFrequency.specificDays
