...
vi.mock('../ColorPicker', () => ({
  ColorPicker: ({ selectedColor, onColorSelect, disabled }: any) => (
    <div data-testid='color-picker'>
      ...
      <button data-testid='color-red' onClick={() => onColorSelect('#FF0000')} disabled={disabled}>
        Red
      </button>
      <button data-testid='color-blue' onClick={() => onColorSelect('#0000FF')} disabled={disabled}>
        Blue
      </button>
    </div>
  ),
...
describe('CreateHabitModal', () => {
  ...
  describe('Loading State', () => {
    it('should disable form inputs when isLoading is true', () => {
      render(<CreateHabitModal {...defaultProps} isLoading />);
