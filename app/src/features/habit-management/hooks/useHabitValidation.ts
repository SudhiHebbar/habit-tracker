...
export const useHabitValidation = (): UseHabitValidationReturn => {
  ...
  const validateField = useCallback(
    (field: string, value: any, _data: UpdateHabitRequest | EditHabitRequest): string | null => {
      switch (field) {
        case 'name':
  ...
  const validateHabit = useCallback(
    (data: UpdateHabitRequest | EditHabitRequest): ValidationError[] => {
      ...
      if (isEditRequest(data)) {
        const hasValidFields = Object.values(data).some(
