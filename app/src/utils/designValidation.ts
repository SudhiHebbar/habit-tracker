...
export function validateSpacingCompliance(
  element: HTMLElement,
  property: string,
...
export function validateTypographyCompliance(
  element: HTMLElement,
  expectedTypography: Partial<DesignTokens['typography']>
): boolean {
  const computedStyle = window.getComputedStyle(element);
...
export function validateResponsiveBreakpoints(): {
  ...
  const tabletQuery = window.matchMedia(
    `(min-width: ${figmaTokens.breakpoints.mobile}) and (max-width: ${figmaTokens.breakpoints.tablet})`
  );
